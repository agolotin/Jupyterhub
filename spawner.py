...
class Spawner(LoggingConfigurable):
    ...
    def add_tutorial_notebooks(self):
        username = self.user.name
        if not os.path.exists('/home/' + username + '/GWASTutorial'):
            #src = "/home/tutorial/*"
            #dest = "/home/" + username + "/tutorial/"
            # Copy all of the contentx of GWAS directory to new user's home
            cmnd = "sudo cp -r --no-preserve=all /home/ubuntu/tutorial/GWASTutorial/ /home/" + username + "/"
            Popen(cmnd.split(' ')).wait()
            # Change permissions on that directory
            uid = getpwnam(username).pw_uid
            path = "/home/" + username
            for root, dirs, files in os.walk(path):  
                for momo in dirs:  
                    os.chown(os.path.join(root, momo), uid, 0)
                for momo in files:
                    os.chown(os.path.join(root, momo), uid, 0)
